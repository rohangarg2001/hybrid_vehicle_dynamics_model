...
class DyanmicsDataset(Dataset):
    def __init__(
        ...
        ],  # data_dict is like instance_name then the modality name and then file name for data amd timestamp
        is_train=False,
    ):
    ...
    def load_data(self):
        ...
        for modality in self.modalities:
            ...
            )  ## how many datapoints for different modalities in dt time
            for single_file_data in self.modality_file_list_data[
                modality
            ]:  # for a modality single_file_data would be data of that modality and instance
                ...
                )  # removing the extra timesteps and making it in an array
                averaged_data = reshaped_data.mean(
                    axis=1
                )  # in dt time we take the average value of the data
                self.modality_processed_data[modality].append(
                    averaged_data
                )  # array of modality data for the entire instance
            self.modality_processed_data[modality] = np.vstack(
                self.modality_processed_data[modality]
            )  # combining array of modality for all instances.
    ...
    def __getitem__(self, idx):
        ...
        #  so idx is the index at take the current state
